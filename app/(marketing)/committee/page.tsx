...
export default function CommitteePage() {
  return <><header className="ll-page-hero"><div className="ll-page-hero-inner"><p className="ll-eyebrow">Governance</p><h1 className="ll-title">The Managing Committee.</h1><p className="ll-lede"><span className="block">Flag-bearers of AMSMA&apos;s shared purpose, the Managing Committee guides the Association&apos;s direction.</span><span className="block mt-1">Together, its members steward responsible practice, knowledge exchange and long-term sector progress.</span></p></div></header><section className="ll-section ll-alt" aria-label="Managing Committee members"><div className="ll-section-inner"><p className="ll-kicker">Formation resolution · 26 May 2026</p><div className="ll-committee-grid">{MEMBERS.map(([photo,name,role]) => <article className="ll-person" key={name}><img className="ll-avatar" src={`/assets/committee/${photo}`} alt="" width="600" height="600" loading="lazy" /><h2 className="ll-heading">{name}</h2><p className="ll-role">{role}</p></article>)}</div></div></section></>;
}
