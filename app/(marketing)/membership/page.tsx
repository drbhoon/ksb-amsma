...
export default function MembershipPage() {
  return <>
    ...
    <section className="ll-section ll-alt"><div className="ll-section-inner"><p className="ll-kicker">Membership categories</p><h2 className="ll-heading">Choose the category that fits.</h2><p className="mt-4 max-w-3xl text-lg">Select a category to see what it includes. You can start your application directly from the open card.</p><div className="grid gap-4 mt-8 md:grid-cols-2">{TIERS_LIST.map((tier, index) => {
        ...
        return <details className="group self-start border border-black/20 bg-[#f2ede1]" key={tier.id} open={index === 0}><summary className="cursor-pointer list-none p-6 [&::-webkit-details-marker]:hidden"><div className="flex items-start justify-between gap-4"><div><p className="ll-kicker">{tier.category}</p><h3 className="text-2xl font-bold leading-tight">{tier.label}</h3></div><span className="relative h-8 w-8 shrink-0 rounded-full border border-black/30" aria-hidden="true"><span className="absolute left-1/2 top-1/2 h-px w-3 -translate-x-1/2 -translate-y-1/2 bg-current"/><span className="absolute left-1/2 top-1/2 h-3 w-px -translate-x-1/2 -translate-y-1/2 bg-current transition-transform group-open:rotate-90"/></span></div><p className="my-3 mb-6">{tier.eligibility}</p><div className="border-t border-black/20 pt-4 text-3xl font-bold">{formatInr(tier.annualFeeRupees)} <span className="font-sans text-xs font-semibold text-black/60">/ year</span></div><p className="mt-2 font-sans text-xs font-semibold text-black/60">Voting weightage: {tier.votingWeightage} · {tier.allowsIndividuals ? 'Individuals eligible' : 'Corporate bodies only'}</p></summary><div className="border-t border-black/20 px-6 pb-6 pt-5"><p className="font-sans text-xs font-bold uppercase tracking-[0.12em] text-[#96501f]">Category benefits</p><p className="mt-2 text-sm">Includes all common member benefits shown below, plus:</p><ul className="mt-5 space-y-3">{[...benefits.highlights, ...benefits.details].map(item => <li className="flex gap-3" key={item}><span className="mt-[0.65em] h-1.5 w-1.5 shrink-0 rounded-full bg-[#96501f]"/><span>{item}</span></li>)}</ul><div className="mt-6 border-t border-black/20 pt-5"><Link className="ll-button ll-button-solid" href="/membership/apply">Apply for Membership</Link></div></div></details>;
      })}</div>
